...
disp(estimates);
[tlifetime, tlifetime_std] = variedIClifetime(estimates,.58,data(:,1));


figure(1);
plot(data(:,1), tlifetime(:,1), '*b', data(:,1), data(:,2), 'ok'); 
figure(2);
plot(data_std(:,1), tlifetime_std(:,1), '*b', data_std(:,1), data_std(:,2), 'ok');
