...
function J = objjNoOutlierLDFit(params)
...
%J = (sum((HDlife(:,2)'-time).^2)) + (sum((HDstd(:,2)'-std).^2)) + (sum((LDlife(:,2)'-time2).^2)) + (sum((LDstd(:,2)'-std2).^2))

J = sum(sum(((HDlife(:,2)'-time).^2)./(HDstd(:,2))))


%+ ((sum((LDlife(:,2)'-time2).^2)) / (sum((LDstd(:,2)))))
